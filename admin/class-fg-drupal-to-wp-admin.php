	...
	class FG_Drupal_to_WordPress_Admin extends WP_Importer {
		...
		public function enqueue_scripts() {
			...
			wp_localize_script( $this->plugin_name, 'objectL10n', array(
				'delete_imported_data_confirmation_message' => __( 'All new imported data will be deleted from WordPress.', 'fg-drupal-to-wp' ),
		...
		public function init() {
			register_importer('fgd2wp', __('Drupal', 'fg-drupal-to-wp'), __('Import a Drupal database into WordPress.', 'fg-drupal-to-wp'), array($this, 'importer'));
		}
		...
		public function dispatch($action) {
			...
			elseif ( !empty($action) ) {
				switch($action) {
					...
					// Delete content
					case 'empty':
						if ( check_admin_referer( 'empty', 'fgd2wp_nonce' ) ) { // Security check
					...
					case 'save':
						...
						// Hook for doing other actions after clicking on the Save button
						do_action('fgd2wp_post_save_button_click');
					...
					case 'modify_links':
						if ( check_admin_referer( 'modify_links', 'fgd2wp_nonce' ) ) { // Security check
							$result = $this->modify_links();
							$this->display_admin_notice(sprintf(_n('%d internal link modified', '%d internal links modified', $result['links_count'], 'fg-drupal-to-wp'), $result['links_count']));
						}
						break;
		...
		private function display_admin_page() {
			...
			$data['description'] = __('This plugin will import articles, stories, pages, images, categories and tags from a Drupal database into WordPress.<br />Compatible with Drupal versions 6, 7 and 8.', 'fg-drupal-to-wp');
		 ...
		 */
		protected function drupal_connect() {
			global $drupal_db;
...
SQL;
		...
		function test_database_connection() {
			...
			if ( $this->drupal_connect() ) {
				try {
					...
					$result = $drupal_db->query("DESC ${prefix}node");
					if ( !is_a($result, 'PDOStatement') ) {
						$errorInfo = $drupal_db->errorInfo();
		 ...
		 */
		public function get_nodes_count($node_type) {
			$prefix = $this->plugin_options['prefix'];
			$sql = "
				SELECT COUNT(*) AS nb
				FROM ${prefix}node n
			...
			if ( $this->drupal_connect() ) {
				...
				if ( !isset($this->premium_options['skip_nodes']) || !$this->premium_options['skip_nodes'] ) {
					...
					if ( !isset($this->premium_options['nodes_to_skip']) || !in_array('article', $this->premium_options['nodes_to_skip']) ) {
						$posts_count = $this->import_nodes('article');
						if ( $posts_count === false ) { // Anti-duplicate
							return;
						}
						$this->display_admin_notice(sprintf(_n('%d article imported', '%d articles imported', $posts_count, 'fg-drupal-to-wp'), $posts_count));
					}
		 ...
		 */
		private function get_taxonomy_term_hierarchy() {
			$hierarchy = array();
			$prefix = $this->plugin_options['prefix'];
		...
		public function import_taxonomies_terms($taxonomy, $taxonomy_name='') {
			...
			$this->log(sprintf(__('Importing %s...', 'fg-drupal-to-wp'), $taxonomy));
			...
			do {
				...
				if ( ($terms != null) && (count($terms) > 0) ) {
					...
					$terms_count += $this->insert_taxonomies_terms($terms, $taxonomy);
				}
			} while ( ($terms != null) && (count($terms) > 0) );
		 ...
		 */
		protected function get_taxonomies_terms($taxonomy, $limit=1000, $taxonomy_name='') {
			$terms = array();
			...
			if ( version_compare($this->drupal_version, '7', '<') ) {
				...
				$sql = "
					...
					$extra_cols
					FROM ${prefix}term_data t
					INNER JOIN ${prefix}vocabulary tv ON tv.vid = t.vid AND tv.`name` = '$taxonomy_name'
			...
			$sql = apply_filters('fgd2wp_get_taxonomies_terms_sql', $sql, $prefix);
		...
		public function insert_taxonomies_terms($terms, $taxonomy) {
			...
			$last_taxonomy_metakey = "fgd2wp_last_taxonomy_${taxonomy}_id";
			$wp_taxonomy = $this->convert_taxonomy($taxonomy);
			...
			foreach ( $terms as $term ) {
				...
				// Parent term
				$parent_id = isset($this->taxonomy_term_hierarchy[$term_id])? $this->taxonomy_term_hierarchy[$term_id]: 0;
				if ( ($parent_id != 0) && isset($this->imported_taxonomies[$parent_id]) ) {
		...
		/**
		 * Convert a Drupal taxonomy to a WordPress taxonomy
		 * 
		 * @param string $taxonomy Drupal taxonomy
		 * @return string WordPress taxonomy
		 */
		private function convert_taxonomy($taxonomy) {
			$wp_taxonomy = '';
			switch ( $taxonomy ) {
				case 'categories':
		...
		public function update_parent_taxonomies_terms($terms, $taxonomy) {
			...
			foreach ( $terms as $term ) {
				$tid = $term['tid'];
				$parent_id = isset($this->taxonomy_term_hierarchy[$tid])? $this->taxonomy_term_hierarchy[$tid]: 0;
				// Parent term
				if ( isset($this->imported_taxonomies[$tid]) && ($parent_id != 0) && isset($this->imported_taxonomies[$parent_id]) ) {
		 ...
		 */
		public function import_nodes($content_type) {
			$imported_nodes_count = 0;

			$plural_content_type_name = (substr($content_type, -1, 1) == 's')? $content_type : $content_type . 's';
			$plural_content_type_name = preg_replace('/ys$/', 'ies', $plural_content_type_name); // "stories" instead of "storys"
			$this->log(sprintf(__('Importing %s...', 'fg-drupal-to-wp'), $plural_content_type_name));
			...
			do {
				...
				$nodes = $this->get_nodes($content_type, $this->chunks_size); // Get the Drupal nodes
				$nodes_count = count($nodes);
				
				if ( is_array($nodes) ) {
					foreach ( $nodes as $node ) {
						if ( $this->import_node($node, $content_type) === false ) {
							return false;
						}
			...
			} while ( ($nodes != null) && ($nodes_count > 0) );
		 ...
		 */
		protected function get_nodes($content_type, $limit=1000) {
			$nodes = array();

			$last_node_type_metakey = "fgd2wp_last_node_${content_type}_id";
			...
			$sql = "
				SELECT n.nid, n.vid, n.title, n.type, n.status, n.created, n.sticky
				$extra_cols
				...
				LIMIT $limit
			";
			$sql = apply_filters('fgd2wp_get_nodes_sql', $sql, $prefix, $last_drupal_id, $limit);
		 ...
		 * @param string $content_type Content type (article, page)
		 * @return int new node ID | false | WP_Error
		 */
		private function import_node($node, $content_type) {
			
			// Anti-duplicate
			if ( !$this->test_antiduplicate ) {
				sleep(2);
				$test_node_id = $this->get_wp_post_id_from_drupal_id($node['nid']);
			...
			if ( !isset($this->premium_options['skip_taxonomies']) || !$this->premium_options['skip_taxonomies'] ) {
				if ( $content_type != 'page' ) { // Pages don't have a category in WordPress
			...
			if ( !$this->plugin_options['skip_media'] ) {
				...
				if ( !$this->plugin_options['only_featured_image'] ) {
					...
					$result = $this->import_media_from_content($body, $post_date);
			...
			// Post type
			$post_type = $this->convert_node_type($content_type);
			...
			if ( is_wp_error($new_post_id) ) {
				...
				add_post_meta($new_post_id, '_fgd2wp_old_node_id', $node['nid'], true);
				
				// Hook for doing other actions after inserting the post
				do_action('fgd2wp_post_insert_post', $new_post_id, $node, $post_type);
			}
		...
		private function get_data_body($node) {
			...
			if ( version_compare($this->drupal_version, '7', '<') ) {
				...
				$extra_criteria = apply_filters('fgd2wp_get_data_field_extra_criteria', '', $node);
				$sql = "
		 ...
		 */
		public function get_node_taxonomies_terms($node_id, $taxonomy='') {
			$terms = array();
		 ...
		 */
		public function convert_node_type($content_type) {
			$post_type = '';
			switch ( $content_type ) {
				case 'article':
				case 'story':
		...
		public function get_and_process_featured_image($node) {
			...
			if ( empty($featured_image) && $this->plugin_options['featured_image'] == 'featured' ) {
				...
				if ( !empty($field_image) ) {
					...
					$featured_image_id = $this->import_media($featured_image['name'], $featured_image['filename'], $featured_image['date'], $featured_image['attributs']);
				}
			}
			...
			if ( empty($featured_image) && $this->plugin_options['featured_image'] != 'none' ) {
				if ( isset($node['body_summary']) && isset($node['body_value']) ) {
					...
					if ( !empty($image_string) ) {
						$result = $this->import_media_from_content($image_string, $post_date);
		 ...
		 */
		private function get_field_image($node_id, $node_type) {
			$image = array();
			...
			if ( $this->table_exists($table_name) ) {
				$sql = "
					SELECT f.fid, i.field_image_alt AS alt, i.field_image_title AS title, f.filename, f.uri, f.filemime, f.${timestamp_field}
		...
		private function set_default_file_paths() {
			...
			// Private path
			$this->file_private_path = trailingslashit($this->get_drupal_variable('file_private_path'));
			...
			// Amazon S3 path
			$this->s3_path = trailingslashit('https://' . $this->get_drupal_variable('amazons3_bucket') . '.s3-' . $this->get_drupal_variable('s3fs_region') . '.amazonaws.com/');
		}
		...
		public function get_path_from_uri($uri) {
			...
			$path = str_replace('s3://', $this->s3_path, $path);
			return $path;
		}
			...
			if ( preg_match_all('#<(img|a)(.*?)(src|href)="(.*?)"(.*?)>#s', $content, $matches, PREG_SET_ORDER) > 0 ) {
				if ( is_array($matches) ) {
					foreach ($matches as $match ) {
						...
						$attachment_id = $this->import_media($image_alt, $filename, $post_date, array(), $options);
		 ...
		 * @param array $attributs Image attributs (image_alt)
		 * @param array $options Options
		 * @return int attachment ID or false
		 */
		public function import_media($name, $filename, $date, $attributs=array(), $options=array()) {
...
//			print "Copy \"$old_filename\" => $new_full_filename<br />";
			...
			// Image Alt
			$image_alt = isset($attributs['image_alt'])? $attributs['image_alt']: '';
			if ( empty($image_alt) && !empty($name) ) {
				$image_alt = wp_strip_all_tags(stripslashes($name), true);
			}
			$description = isset($attributs['description'])? $attributs['description']: '';
			
			// GUID
			$upload_dir = wp_upload_dir();
			$guid = str_replace($upload_dir['basedir'], $upload_dir['baseurl'], $new_full_filename);
			
			$attachment_id = $this->insert_attachment($post_title, $basename, $new_full_filename, $guid, $date, $filetype['type'], $image_alt, $description);
		 ...
		 */
		public function insert_attachment($attachment_title, $basename, $new_full_filename, $guid, $date, $filetype, $image_alt='', $description='') {
						...
						if ( is_array($matches) ) {
							foreach ( $matches as $match ) {
								...
								if ( $this->is_internal_link($link_without_anchor) ) {
									...
									$linked_object = $this->get_wp_object_from_drupal_url($link_without_anchor);
									
									if ( is_a($linked_object, 'WP_Post') ) {
										// Post found
										$linked_post_id = $linked_object->ID;
										$linked_post_id = apply_filters('fgd2wp_post_get_post_by_drupal_id', $linked_post_id, $post); // Used to get the ID of the translated post
										...
										$linked_term_id = apply_filters('fgd2wp_post_get_term_by_drupal_id', $linked_term_id, $post); // Used to get the ID of the translated term
		...
		private function drupal_version() {
			$version = '';
			if ( $this->table_exists('taxonomy_term_field_data') ) {
				$version = '8';
			} elseif ( $this->table_exists('taxonomy_term_data') ) {
				$version = '7';
			} else {
				$version = '6';
			}
		 ...
		 */
		public function get_wp_post_id_from_drupal_id($drupal_id) {
			$post_id = $this->get_wp_post_id_from_meta('_fgd2wp_old_node_id', $drupal_id);
			return $post_id;
		}
